...
# --- CSS INSTITUCIONAL (AZUL COPPEL - SIN ROJOS) ---
st.markdown(
    """
    <style>
...
# --- BASE DE DATOS (PASO 2 INTEGRADO AQUÍ) ---
def obtener_conexion():
  return sqlite3.connect("db_coppel_v5.db", check_same_thread=False)
...
def inicializar_db():
  ...
  cursor.execute("""
        CREATE TABLE IF NOT EXISTS proyectos (
            ...
            area_negocio TEXT, tipo_proyecto TEXT, subtipo TEXT, gerente TEXT, 
            lider_asignado TEXT, etapa_actual TEXT, estatus_tiempo TEXT, avance_real REAL, 
  ...
  # 3. PASO 2: Tabla de tareas para el Plan de Trabajo con Predecesoras (Gantt)
  cursor.execute("""
  ...
  if cursor.fetchone()[0] == 0:
    ...
    cursor.execute(
        "INSERT INTO usuarios VALUES ('ivan.salazar@coppel.com', 'Coppel2026',"
        " 'Usuario', 'Oscar Ivan Salazar')"
    )
...
def obtener_lista_usuarios():
  ...
  df_users = pd.read_sql_query(
      "SELECT nombre FROM usuarios ORDER BY nombre ASC", conn
  )
  conn.close()
...
def calcular_fechas_tarea(proyecto_id):
  ...
  df_t = pd.read_sql_query(
      "SELECT * FROM tareas WHERE proyecto_id = ? ORDER BY id ASC",
      conn,
      params=(proyecto_id,),
  )
  conn.close()
...
if not st.session_state.autenticado:
  ...
  with col_centro:
    ...
    st.write("")
    st.markdown(
        "<h1 style='text-align: center; color:#081754 !important;'>Portafolio"
        " de Incentivos</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align: center; color: #4A4A4A; margin-bottom:"
        " 30px;'>Coppel</p>",
        unsafe_allow_html=True,
    )
    ...
    with st.form("login_form"):
      st.markdown(
          "<h3 style='color:#05297A !important; font-size: 1.1rem;'>Iniciar"
          " Sesión</h3>",
          unsafe_allow_html=True,
      )
      ...
      if submit:
        if correo_input.strip() == "":
          ...
          cursor = conn.cursor()
          cursor.execute(
              "SELECT password, rol, nombre FROM usuarios WHERE LOWER(correo)=?",
              (correo_input.strip().lower(),),
          )
...
with st.sidebar:
  st.markdown(
      f"<h3 style='margin-bottom:0; font-size:1.1rem;"
      f" color:#081754;'>{st.session_state.nombre_actual}</h3>",
      unsafe_allow_html=True,
  )
  st.markdown(
      f"<p style='color:#4A4A4A; font-size:0.85rem;"
      f" margin-top:0;'>{st.session_state.correo_actual}</p>",
      unsafe_allow_html=True,
  )

  badge_color = "status-green" if es_moderador else "status-gray"
  st.markdown(
      f"<div><span class='status-badge"
      f" {badge_color}'>{st.session_state.rol}</span></div>",
      unsafe_allow_html=True,
  )
      ...
      if st.form_submit_button("Actualizar", use_container_width=True):
        if nueva_pass == confirmar_pass and nueva_pass:
          ...
          cursor.execute(
              ...
              (nueva_pass, st.session_state.correo_actual),
          )
          conn.commit()
          conn.close()
          st.success("Guardado.")
...
# --- HEADER Y MÉTRICAS ---
st.markdown(
    "<h2 style='margin-bottom: 20px; color:#081754;'>Visión General</h2>",
    unsafe_allow_html=True,
)
...
if not df.empty:
  ...
  prom_avance = df["avance_real"].mean() * 100
...
if es_moderador and not df.empty:
  ...
  def generar_pdf_ejecutivo(dataframe):
    ...
    pdf.cell(
        0,
        8,
        "Reporte Ejecutivo de Portafolio - Incentivos Coppel",
        ln=True,
        align="L",
    )
    ...
    pdf.cell(
        0,
        6,
        f"Fecha de emisión: {fecha_emision} | Generado por:"
        f" {st.session_state.nombre_actual}",
        ln=True,
        align="L",
    )
    pdf.ln(4)
    ...
    for _, row in dataframe.iterrows():
      ...
      pdf.set_font("Arial", "B", 8)
      pdf.cell(
          42, 7, str(row["ultima_actualizacion"] or "Sin registro")[:19], 1, 0, "C"
      )
      pdf.set_font("Arial", "", 8)
  ...
  with st.sidebar:
    ...
    with open(pdf_path, "rb") as file:
      st.download_button(
          ...
          data=file,
          file_name=(
              f"Reporte_Portafolio_{datetime.now().strftime('%Y%m%d')}.pdf"
          ),
...
# --- PESTAÑAS DINÁMICAS ---
if es_moderador:
  tabs = st.tabs(["Seguimiento Operativo", "Nuevo Proyecto", "Accesos"])
else:
  tabs = st.tabs(["Seguimiento Operativo", "Nuevo Proyecto"])

lista_lideres_registrados = obtener_lista_usuarios()

# PESTAÑA 1: VISOR + PLAN DE TRABAJO ESTILO PLANVIEW
with tabs[0]:
  if df.empty:
    st.info("El portafolio está vacío.")
  else:
    st.markdown(
        "<h4 style='color:#05297A; margin-bottom:10px;'>🔍 Filtros de"
        " Búsqueda</h4>",
        unsafe_allow_html=True,
    )
    ...
    txt_busqueda = f_col1.text_input(
        "Buscar por Folio o Nombre", placeholder="ej. INC-001 o Banca"
    )
    filtro_area = f_col2.selectbox(
        "Área del Negocio", ["Todas las Áreas"] + OPCIONES_AREAS
    )
    filtro_estatus = f_col3.selectbox(
        "Estatus de Tiempo", ["Todos los Estatus"] + OPCIONES_ESTATUS
    )
    filtro_lider = f_col4.selectbox(
        "Responsable", ["Todos los Responsables"] + lista_lideres_registrados
    )
    ...
    st.caption(
        f"📌 Mostrando **{len(df_filtrado)}** de **{len(df)}** iniciativas en el"
        " portafolio."
    )
    st.divider()
    ...
    if df_filtrado.empty:
      st.warning(
          "No se encontraron iniciativas que coincidan con los filtros"
          " aplicados."
      )
    else:
      for _, row in df_filtrado.iterrows():
        ...
        with st.expander(
            f"{row['folio'] or 'S/F'} | {row['nombre']} — Avance Global:"
          ...
          with st.form(f"update_{p_id}"):
            ...
            c1.markdown(
                ...
                f" style='color:#4A4A4A;'>Responsable:</span> <b"
                f" style='color:#081754;'>{row['lider_asignado']}</b></p>",
                unsafe_allow_html=True,
            )
            c2.markdown(
                f"<p style='margin:0; font-size:0.88rem;'><span"
                f" style='color:#4A4A4A;'>Área:</span> <b"
                f" style='color:#081754;'>{row['area_negocio']}</b></p>",
                unsafe_allow_html=True,
            )
            c3.markdown(
                f"<p style='margin:0; font-size:0.88rem; text-align:right;'><span"
                " style='color:#4A4A4A;'>Última Actualización:</span> <b"
                f" style='color:#05297A;'>{row['ultima_actualizacion'] or 'Sin registro'}</b></p>",
                unsafe_allow_html=True,
            )
            st.divider()
            ...
            u_resumen = st.text_area(
                "Bitácora de Estatus / Comentarios",
                row["resumen_estatus"] or "",
                height=80,
            )
            ...
            u_plan = l2.text_input(
                "Plan de Trabajo Excel (URL)", row["plan_url"] or ""
            )
            ...
            if btn1.form_submit_button("Guardar Cambios", type="primary"):
              ...
              cursor = conn.cursor()
              cursor.execute(
                  "UPDATE proyectos SET etapa_actual=?, estatus_tiempo=?,"
                  " avance_real=?, lider_asignado=?, resumen_estatus=?,"
                  " carpeta_url=?, plan_url=?, ultima_actualizacion=? WHERE"
                  " id=?",
                  (
                      u_etapa,
                      u_estatus,
          ...
          # --- SECCIÓN DEL PLAN DE TRABAJO CON PREDECESORAS (PASO 3) ---
          st.divider()
          st.markdown(
              "<h4 style='color:#05297A;'>📅 Plan de Trabajo y Cronograma"
              " (Gantt)</h4>",
              unsafe_allow_html=True,
          )
          ...
          if not df_tareas.empty:
            fig = px.timeline(
                ...
                title="Cronograma WBS de Tareas y Dependencias",
            )
            fig.update_yaxes(autorange="reversed")
            fig.update_layout(
                height=250 + (len(df_tareas) * 25),
                xaxis_title="Fechas",
                yaxis_title="Tareas",
          ...
          with st.expander("➕ Agregar Tarea al Cronograma"):
            with st.form(f"form_tarea_{p_id}", clear_on_submit=True):
              ...
              t_duracion = col_t4.number_input(
                  "Duración (Días)", min_value=1, value=5
              )
              t_preds = col_t5.text_input(
                  "Predecesoras (IDs ej. 1, 2)",
                  help="Escribe los IDs de las tareas que deben terminar antes.",
              )

              if st.form_submit_button("Guardar Tarea"):
                if t_nombre.strip():
                  cursor = conn.cursor()
                  f_fin_est = pd.to_datetime(t_f_inicio) + timedelta(
                      days=t_duracion - 1
                  )
                  cursor.execute(
                      """
...
with tabs[1]:
  if es_moderador:
    ...
    with st.form("form_nuevo", clear_on_submit=True):
      st.markdown(
          "<h4 style='color:#05297A;'>Alta de Iniciativa</h4>",
          unsafe_allow_html=True,
      )
      c1, c2, c3 = st.columns(3)
      ...
      if st.form_submit_button("Registrar Proyecto", type="primary"):
        if nombre.strip():
          cursor = conn.cursor()
          cursor.execute(
              "INSERT INTO proyectos (folio, nombre, area_negocio, tipo_proyecto,"
              " subtipo, gerente, lider_asignado, etapa_actual,"
              " estatus_tiempo, avance_real) VALUES (?, ?, ?, ?, ?, ?, ?, ?,"
              " ?, 0)",
              (
                  folio,
                  nombre,
...
if es_moderador:
  with tabs[2]:
    st.write("")
    df_users = pd.read_sql_query(
        "SELECT nombre as Colaborador, correo as Correo, rol as Permisos FROM"
        " usuarios",
        conn,
    )
    ...
    with c_add:
      with st.form("form_alta_usuario", clear_on_submit=True):
        st.markdown(
            "<h4 style='color:#05297A;'>Dar de Alta</h4>",
            unsafe_allow_html=True,
        )
          ...
          if n_correo and n_pass and n_nombre:
            try:
              ...
              cursor.execute(
                  ...
                  (n_correo.strip().lower(), n_pass, n_rol, n_nombre),
              )
              conn.commit()
    ...
    with c_del:
      with st.form("form_baja_usuario"):
        st.markdown(
            "<h4 style='color:#081754;'>Revocar Acceso</h4>",
            unsafe_allow_html=True,
        )
        ...
        correo_borrar = st.selectbox(
            "Colaborador a eliminar", ["Seleccionar..."] + lista_correos
        )
        st.write("")
        st.write("")
        st.write("")
        if st.form_submit_button("Eliminar Cuenta", type="secondary"):
          if correo_borrar != "Seleccionar...":
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM usuarios WHERE correo=?", (correo_borrar,)
            )
            conn.commit()
