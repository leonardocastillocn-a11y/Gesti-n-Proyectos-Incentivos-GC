...
st.set_page_config(page_title="Portafolio de Incentivos Coppel", page_icon="💼", layout="wide", initial_sidebar_state="expanded")
...
def inicializar_db():
  ...
  with engine.begin() as conn:
    ...
    # NUEVA TABLA: Historial de Bitácora
...
def obtener_lista_usuarios():
  ...
  return df["nombre"].tolist() if not df.empty else ["Leonardo Castillo", "Oscar Ivan Salazar"]
...
def calcular_fechas_tarea(proyecto_id):
  ...
  df_t["fecha_inicio"] = pd.to_datetime(df_t["fecha_inicio"]); df_t["fecha_fin"] = pd.to_datetime(df_t["fecha_fin"])
  fechas_fin = {}
  for idx, row in df_t.iterrows():
    ...
    df_t.at[idx, "fecha_fin"] = fin; fechas_fin[t_id] = fin
  return df_t
...
if not df.empty:
    ...
    if not proyectos_retrasados.empty and "alerta_mostrada" not in st.session_state:
        st.toast(f"Tienes {len(proyectos_retrasados)} iniciativas requiriendo atención urgente.", icon="🚨")
        st.session_state.alerta_mostrada = True

# --- SIDEBAR MEJORADO (CON EXCEL) ---
with st.sidebar:
  st.markdown(f"<div style='background-color:#F8FAFC; padding: 20px; border-radius: 12px; border: 1px solid #E2E8F0; margin-bottom: 20px;'><h3 style='margin:0 0 5px 0; font-size:1.1rem; color:#0F172A;'>👤 {st.session_state.nombre_actual}</h3><p style='margin:0 0 10px 0; color:#64748B; font-size:0.8rem;'>{st.session_state.correo_actual}</p><span class='status-badge {'status-green' if es_moderador else 'status-gray'}'>{st.session_state.rol}</span></div>", unsafe_allow_html=True)
  ...
  if not df.empty:
      ...
      def generar_pdf(dataframe):
        pdf = FPDF(orientation="L", unit="mm", format="A4"); pdf.set_auto_page_break(auto=True, margin=15); pdf.add_page(); pdf.set_font("Arial", "B", 16); pdf.set_text_color(5, 41, 122); pdf.cell(0, 8, "Reporte Ejecutivo de Portafolio", ln=True, align="L"); pdf.set_font("Arial", "", 9); pdf.set_text_color(100, 100, 100); pdf.cell(0, 6, f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}", ln=True, align="L"); pdf.ln(4); pdf.set_font("Arial", "B", 8); pdf.set_fill_color(241, 245, 249); pdf.set_text_color(8, 23, 84); pdf.cell(22, 8, "Folio", 1, 0, "C", True); pdf.cell(60, 8, "Nombre del Proyecto", 1, 0, "L", True); pdf.cell(30, 8, "Área", 1, 0, "L", True); pdf.cell(35, 8, "Responsable", 1, 0, "L", True); pdf.cell(25, 8, "Estatus", 1, 0, "C", True); pdf.cell(18, 8, "Avance", 1, 0, "C", True); pdf.cell(42, 8, "Ult. Act.", 1, 0, "C", True); pdf.cell(45, 8, "Etapa Actual", 1, 1, "L", True); pdf.set_font("Arial", "", 8); pdf.set_text_color(40, 40, 40)
      ...
      st.download_button("Descargar Base (Excel)", data=excel_buffer.getvalue(), file_name=f"Base_Datos_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="secondary")
...
st.markdown("<h2 style='margin-bottom: 25px; color:#0F172A;'>📊 Visión General del Portafolio</h2>", unsafe_allow_html=True)
if not df.empty and not proyectos_retrasados.empty:
    st.markdown(f"<div style='background-color:#FEF2F2; border-left: 4px solid #EF4444; padding: 15px; border-radius: 8px; margin-bottom: 20px;'><p style='color:#991B1B; margin:0; font-weight:600;'>⚠️ Atención Crítica: Tienes {len(proyectos_retrasados)} proyectos marcados como Retrasados o Detenidos.</p></div>", unsafe_allow_html=True)
...
# --- PESTAÑAS (TABS) ---
if es_moderador: tabs = st.tabs(["📈 Dashboard Analítico", "🚀 Seguimiento (Lista)", "📋 Kanban", "➕ Nuevo Proyecto", "👥 Accesos"])
else: tabs = st.tabs(["📈 Dashboard Analítico", "🚀 Seguimiento (Lista)", "📋 Kanban", "➕ Nuevo Proyecto"])
...
# PESTAÑA 2: SEGUIMIENTO OPERATIVO (LISTA + BITÁCORA HISTÓRICA)
with tabs[1]:
  if df.empty: st.info("El portafolio está vacío.")
    ...
    st.markdown(f"<p style='color: #64748B; font-size: 0.9rem; margin-top: 10px;'>📌 Mostrando <b>{len(df_filtrado)}</b> iniciativas.</p>", unsafe_allow_html=True)
    ...
    for _, row in df_filtrado.iterrows():
      ...
      with st.expander(f"{row['folio'] or 'S/F'} | {row['nombre']} — Avance: {int((row['avance_real'] or 0)*100)}%"):
        ...
        with st.form(f"update_{p_id}"):
          ...
          u_lider = st.selectbox("Reasignar Líder de Proyecto", lista_lideres_registrados, index=(lista_lideres_registrados.index(row["lider_asignado"]) if row["lider_asignado"] in lista_lideres_registrados else 0)) if es_moderador else row["lider_asignado"]
          
          # Historial de Bitácora
          st.markdown("<p style='font-size:0.85rem; font-weight:600; color:#334155; margin-bottom:5px;'>NUEVO COMENTARIO DE BITÁCORA</p>", unsafe_allow_html=True)
          u_comentario = st.text_input("Agregar actualización (Se guardará en el historial)", placeholder="Escribe el estatus de la semana...")
          ...
          if btn1.form_submit_button("Actualizar Proyecto", type="primary"):
            ...
            with engine.begin() as conn:
              conn.execute(sqlalchemy.text("UPDATE proyectos SET etapa_actual=:e, estatus_tiempo=:s, avance_real=:a, lider_asignado=:l, carpeta_url=:c, plan_url=:p, ultima_actualizacion=:u WHERE id=:id"), {"e": u_etapa, "s": u_estatus, "a": u_avance, "l": u_lider, "c": u_carpeta, "p": u_plan, "u": ahora, "id": p_id})
              if u_comentario.strip():
                  conn.execute(sqlalchemy.text("INSERT INTO bitacora (proyecto_id, usuario_nombre, fecha_hora, comentario) VALUES (:p_id, :usr, :fh, :com)"), {"p_id": p_id, "usr": st.session_state.nombre_actual, "fh": ahora, "com": u_comentario.strip()})
            st.rerun()

          if es_moderador and btn2.form_submit_button("Eliminar", type="secondary"):
            with engine.begin() as conn: conn.execute(sqlalchemy.text("DELETE FROM proyectos WHERE id=:id"), {"id": p_id})
            st.rerun()

        # Mostrar Historial de Bitácora
        historial_proyecto = df_bitacora[df_bitacora['proyecto_id'] == p_id]
        if not historial_proyecto.empty:
            st.markdown("<h5 style='margin-top: 15px; color:#0F172A;'>📜 Historial de Cambios</h5>", unsafe_allow_html=True)
        ...
        if not df_tareas.empty:
          fig = px.timeline(df_tareas, x_start="fecha_inicio", x_end="fecha_fin", y="nombre_tarea", color="porcentaje_avance", color_continuous_scale=[[0, "#E2E8F0"], [0.5, "#FCD34D"], [1, "#05297A"]], hover_data=["responsable"])
          fig.update_yaxes(autorange="reversed"); fig.update_layout(height=200 + (len(df_tareas) * 20), margin=dict(l=0, r=0, t=10, b=0))
...
# PESTAÑA 3: VISTA KANBAN (TRELLO)
with tabs[2]:
    st.write("")
    if df.empty:
        st.info("Agrega proyectos para verlos en el tablero Kanban.")
    else:
        # Creamos columnas para el tablero
        k_cols = st.columns(len(OPCIONES_ESTATUS))
        for i, status in enumerate(OPCIONES_ESTATUS):
            with k_cols[i]:
                # Cabecera de la columna Kanban
                st.markdown(f"<div style='background-color:#F8FAFC; padding:10px; border-radius:8px; border:1px solid #E2E8F0; text-align:center; font-weight:700; color:#0F172A; margin-bottom:15px;'>{status.upper()}</div>", unsafe_allow_html=True)
...
with tabs[3]:
  ...
  if es_moderador:
    with st.form("f_nuevo", clear_on_submit=True):
      ...
      estatus_inicial = c9.selectbox("Estado Inicial", OPCIONES_ESTATUS, index=0)
      if st.form_submit_button("Crear Proyecto", type="primary"):
        if nombre.strip():
          ...
          st.success("Iniciativa creada."); st.rerun()
        else: st.error("Nombre obligatorio.")

# PESTAÑA 5: USUARIOS (Si es moderador)
if es_moderador:
  with tabs[4]:
    st.write("")
